...
async function initializeAndUseQuestions() {
    ...
    if (questions.length > 0) {
        ...
        alert(`Loaded ${questions.length} questions successfully! Check the console.`);
    } else {
...
let currentFilters = { subject: 'all', difficulty: 'all' }; 
...
function populateSubjectStatsTable() {
    ...
    // Example data (replace with real stats from Firebase or userAnswers)
    const subjectStats = [
        { subject: "Pharmacology", attempted: 100, correct: 80, accuracy: 80 },
        { subject: "Pathology", attempted: 90, correct: 65, accuracy: 72 },
        { subject: "Anatomy", attempted: 80, correct: 52, accuracy: 65 },
        { subject: "Physiology", attempted: 70, correct: 61, accuracy: 87 }
    ];
...
function updateLoginUI(isLoggedIn) {
     if (loginBtn && userInfo) {
         ...
         userInfo.classList.toggle('flex', isLoggedIn); // Ensure display: flex is used when visible
     }
     if (isLoggedIn) {
        // Update username display if needed
        // document.getElementById('username').textContent = 'User Name'; 
        closeLoginModal(); // Close modal if open after successful login
     }
}
...
/** Initializes the Subjects Performance Chart */
function initSubjectsChart() {
    if (!subjectsChartCanvas) return;
    const ctx = subjectsChartCanvas.getContext('2d');
    new Chart(ctx, {
        type: 'doughnut',
        data: {
            labels: ['Pharmacology', 'Pathology', 'Anatomy', 'Physiology', 'Radiology'], // Placeholder
            datasets: [{
                label: 'Questions by Subject',
                data: [85, 92, 77, 63, 48], // Placeholder data
                backgroundColor: [
                    'rgba(44, 122, 123, 0.7)',
                    'rgba(72, 187, 180, 0.7)',
                    'rgba(129, 230, 217, 0.7)',
                    'rgba(178, 245, 234, 0.7)',
                    'rgba(45, 190, 165, 0.7)'
                ],
                borderColor: '#fff',
                borderWidth: 1
